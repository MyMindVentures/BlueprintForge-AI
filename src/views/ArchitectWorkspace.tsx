...
const ArchitectWorkspace: React.FC = () => {
  ...
  const objects = [
    { id: 'OBJ-001', name: 'Identity Vault', type: 'Security Object', status: 'ready' },
    { id: 'OBJ-002', name: 'Transaction Bus', type: 'Logic Module', status: 'ready' },
    { id: 'OBJ-003', name: 'Agreement Gate', type: 'Access Protocol', status: 'draft' },
  ];
  ...
  const send = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        text: "Analyzing market gaps in " + input + "... I've identified 4 high-value object modules for this architecture." 
      }]);
    }, 1000);
    setInput('');
  };
    ...
    <div className="h-full flex flex-col gap-6 animate-in fade-in duration-700">
      ...
      <div className="flex justify-between items-center bg-bg-surface border border-border-soft p-6 rounded-3xl shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center gap-3 relative z-10">
           ...
           <button className="px-6 py-2.5 bg-accent-blue text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-xl shadow-accent-blue/20 flex items-center gap-2 hover:bg-accent-blue/90 transition-all active:scale-95">
        ...
        <div className="col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 no-scrollbar">
           <div className="bg-bg-surface border border-border-soft rounded-2xl p-6 space-y-5 shadow-xl">
              ...
              <div className="space-y-3">
                 {objects.map(obj => (
                   <div key={obj.id} className="p-4 bg-bg-surface-elevated border border-border-soft rounded-2xl hover:border-accent-cyan/50 transition-all cursor-pointer group shadow-inner">
           ...
           <div className="flex-1 bg-bg-surface border border-border-soft rounded-3xl shadow-2xl overflow-hidden flex flex-col relative min-h-0">
              <div className="border-b border-border-soft p-4 flex items-center justify-between bg-white/5 backdrop-blur-md">
                 ...
                 <div className="flex gap-2">
                    ...
                    <button className="px-4 py-1.5 text-[10px] font-black bg-text-primary text-bg-main rounded-xl flex items-center gap-2 hover:bg-text-secondary transition-all shadow-lg shadow-white/5">
                       <RefreshCw className="w-3.5 h-3.5" /> RE-CALCULATE BLOCK
                    </button>
                 </div>
              </div>
              ...
              <div className="flex-1 p-12 overflow-y-auto no-scrollbar font-sans leading-relaxed text-text-secondary space-y-10">
                 <header className="space-y-6">
                    <div className="flex items-center gap-3">
                       ...
                       <h1 className="text-5xl font-black italic tracking-tighter text-text-primary">System Logic Map</h1>
                    </div>
                    <p className="text-xl text-text-muted font-serif italic max-w-2xl leading-snug">Structural definition of the application core to ensure high-readiness for builder implementation.</p>
                 </header>
                 ...
                 <div className="p-10 bg-bg-surface-elevated rounded-3xl border border-border-soft relative overflow-hidden group shadow-2xl">
                    ...
                    <div className="relative z-10 space-y-8">
                       <div className="flex items-center gap-4 border-b border-white/5 pb-4">
                          ...
                          <h3 className="font-black text-2xl tracking-tight text-text-primary uppercase tracking-widest">OBJ-001: Profile Architecture</h3>
                       </div>
